...
class CampaignsUseCase {
  ...
  CampaignsUseCase(this._medusaAdminV2);

  static CampaignsUseCase get instance => getIt<CampaignsUseCase>();
  ...
  Future<Result<CampaignDeleteRes, MedusaError>> deleteCampaign(
      {required String id}) async {
    try {
      final result = await _campaignsRepository.delete(id);
      return Success(result);
    } on DioException catch (e) {
      return Error(MedusaError.fromHttp(
        status: e.response?.statusCode,
