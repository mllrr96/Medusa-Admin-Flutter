...
import 'package:medusa_admin/src/features/store_settings/domain/usecases/sales_channel/sales_channel_crud_use_case.dart';
...
class SalesChannelCrudBloc
  ...
  void _loadAll(_LoadAll event, Emitter<SalesChannelCrudState> emit) async {
    ...
    result.when(
        (success) => emit(_SalesChannels(success.salesChannels, success.count)),
        (error) => emit(_Error(error)));
  }

  void _create(_Create event, Emitter<SalesChannelCrudState> emit) async {
    emit(const _Loading());
    final result = await salesChannelCrudUseCase.create(event.payload);
    result.when((success) => emit(_SalesChannel(success)),
        (error) => emit(_Error(error)));
  }
